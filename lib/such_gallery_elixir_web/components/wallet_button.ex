      ...
      <script>
        ...
        function renderLoggedIn(btn, user) {
          ...
          btn.innerHTML = `
            <div class="flex items-center gap-2">
              ...
              <button onclick="handleLogout()" class="text-xs text-gray-400 hover:text-red-500 transition-colors">✕</button>
            </div>
          `;
        }
        ...
        function renderLoggedOut(btn) {
          if (window.SuchGallery && SuchGallery.walletAvailable()) {
            btn.innerHTML = `
              <button onclick="handleSignIn()" class="px-3 py-1.5 text-sm font-medium text-white bg-gray-900 rounded-lg hover:bg-gray-700 transition-colors cursor-pointer">
                Connect Wallet
              </button>
            `;
          } else {
            btn.innerHTML = `
              <a href="https://metamask.io/download/" target="_blank" class="px-3 py-1.5 text-sm font-medium text-gray-600 border border-gray-300 rounded-lg hover:border-gray-500 transition-colors">
                Install MetaMask
              </a>
            `;
          }
